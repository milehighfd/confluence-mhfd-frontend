...
export const getCurrentProjectStatus = (project:any) => {
  console.log('project about to find status', project, project?.project_statuses?.find((ps:any) => ps.project_status_id === project.current_project_status_id));
  // const current = project?.project_statuses?.find((ps:any) => ps.project_status_id === project.current_project_status_id); 
  const current = project?.currentId[0];
  return current;
}
