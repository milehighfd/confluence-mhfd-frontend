    ...
    return <div id={"popup-" + id} className="map-pop-00">
      <Card hoverable>
        ...
        { !ep && <div style={{ padding: '10px', marginTop: '-15px', color: '#28C499', display:'flex'}}>
            ...
            <Button id={"buttonPopup-" + id} style={{ width: item.type == 'project'? '100%' : '50%', color: '#28C499' }} onClick={() => test(item)} className="btn-borde">See Details</Button>
        </div>} 
        { ep && <div style={{ padding: '10px', marginTop: '-15px', color: '#28C499', display:'flex'}}>
            <Button id={"buttonEdit-" + id} style={{ width: item.type == 'project'? '100%' : '50%', color: '#28C499' }} onClick={() => test(item)} className="btn-borde">Edit Project</Button>
        </div>}
      </Card>
    </div>
};
