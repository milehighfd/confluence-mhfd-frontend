      ...
      items={[
        {
          ...
          children: GROUPS.map((gb, index) => {
            return {
              ...
              onClick={() => { gb !== 'MHFD Lead' ? setCurrentGroup(gb.toLowerCase().replace(' ', '')) : setCurrentGroup('staff'); setActiveDrop(index); setOpenDrop(false); setOpenGroups([true])}}>{gb === 'Jurisdiction' ? 'Local Government' : gb}</div>,
