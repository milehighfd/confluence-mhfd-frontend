...
const loadComponentPopup = (index: number, item: any, isComponent: boolean, userInformation: any, maptype: any, eventFunctions: any) => (
  <>
...
export const loadIconsPopup = (menu: any, popups: any, index: any, showPopup: any, ids: any) => {
  ...
  switch (true) {
    case menu.includes('roblem'):
      ...
      break;
    case menu.includes('roject') && popup.projecctype !== undefined && (popup.mapType==='WORKREQUEST' && (popup.status === 'Active' || popup.status === 'Approved' || popup.status === 'Requested' || popup.status === 'Submitted')):
      iconLocation = `/Icons/icon-projects-${popup.status.toLowerCase()}.png`;
      break;
    case menu.includes('roject') && popup.projecctype !== undefined && (popup.projecctype === NEW_PROJECT_TYPES.MAINTENANCE_SUBTYPES.Debris_Management || popup.projecctype === NEW_PROJECT_TYPES.MAINTENANCE_SUBTYPES.Vegetation_Management || popup.projecctype === NEW_PROJECT_TYPES.MAINTENANCE_SUBTYPES.Sediment_Removal || popup.projecctype === NEW_PROJECT_TYPES.MAINTENANCE_SUBTYPES.Minor_Repairs || popup.projecctype === NEW_PROJECT_TYPES.MAINTENANCE_SUBTYPES.Restoration || popup.projecctype.includes(NEW_PROJECT_TYPES.Maintenance) || popup.projecctype.includes('Capital') || popup.projecctype === "Fee in Lieu"): 
    ...
    case menu === "NCRS Soils" && popup.hydgrpdcd !== undefined && (popup.hydgrpdcd === 'D'):
      ...
      break;
    case menu === "FEMA Flood Hazard" && popup.fld_zone !== undefined && (popup.fld_zone === 'AE' && popup.zone_subty === '-'):
      iconLocation = '/Icons/ic_FEMA_ZoneAE@2x.png';
      break;
    case menu === "FEMA Flood Hazard" && popup.fld_zone !== undefined && (popup.fld_zone === 'AE' && popup.zone_subty === 'FLOODWAY'):
  ...
  if (menu) {
    return (
      <Button id={'menu-' + index} onClick={() => showPopup(index, popups.length, ids[index])} className="btn-transparent"><img style={{ width: '18px', borderRadius: '2px' }} src={iconLocation} alt="" /><span className="text-popup-00"> {menu}</span> <RightOutlined /></Button>
    )
  }
};
